...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    SSJHTTPSessionModel *currenModel=[self modelForTableView:tableView atIndexPath:indexPath];
    ...
    requestHTTPMethod = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@   %@   %@",currenModel.requestHTTPMethod,currenModel.responseMIMEType,[((SSJHTTPSessionModel *)((httpRequests)[indexPath.row])).startDateString substringFromIndex:5]]
                                                           attributes:@{
                                                                        NSFontAttributeName : detailFont,
                                                                        NSForegroundColorAttributeName: detailColor
                                                                        }];
...
- (void)updateSearchResultsWithSearchString:(NSString *)searchString {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSArray *tempFilterHTTPRequests = [httpRequests filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(SSJHTTPSessionModel *httpRequest, NSDictionary *bindings) {
...
- (SSJHTTPSessionModel *)modelForTableView:(UITableView *)tableView atIndexPath:(NSIndexPath *)indexPath {
    SSJHTTPSessionModel *currenModel=[[SSJHTTPSessionModel alloc] init];
    if (tableView == mySearchDisplayController.searchResultsTableView) {
        currenModel=(SSJHTTPSessionModel *)((filterHTTPRequests)[indexPath.row]);
    }else{
        currenModel=(SSJHTTPSessionModel *)((httpRequests)[indexPath.row]);
    }
    return currenModel;
}
